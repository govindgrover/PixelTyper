...
class ImagePreviewWidget(ctk.CTkFrame):
	...
	def __init__(self, master, **kwargs):
		...
		self.preview_label = ctk.CTkLabel(self, text="No image loaded", fg_color="gray20")
...
class SimpleOverlayTab(ctk.CTkFrame):
	...
	def __init__(self, master, preview_widget, **kwargs):
		super().__init__(master, **kwargs)
		...
		ctk.CTkLabel(img_frame, text="Image:").pack(side="left", padx=5)
		self.img_path_label = ctk.CTkLabel(img_frame, text="No image selected", anchor="w")
		self.img_path_label.pack(side="left", fill="x", expand=True, padx=5)
		ctk.CTkButton(img_frame, text="Browse", command=self.browse_image, width=100).pack(side="right", padx=5)
		
		# Text input
		text_frame = ctk.CTkFrame(self)
		text_frame.grid(row=1, column=0, padx=20, pady=10, sticky="ew")
		
		ctk.CTkLabel(text_frame, text="Text:").pack(anchor="w", padx=5, pady=5)
		...
		ctk.CTkLabel(pos_frame, text="Position:").pack(anchor="w", padx=5)
		...
		ctk.CTkButton(coord_frame, text="Click to Select", command=self.select_coordinates).pack(side="left", padx=10)
		...
		ctk.CTkLabel(style_inner, text="Font Size:").pack(side="left", padx=5)
		...
		ctk.CTkLabel(style_inner, text="Color:").pack(side="left", padx=10)
		...
		ctk.CTkButton(style_inner, text="Pick", width=50, command=self.pick_color).pack(side="left", padx=5)
		
		# Apply button
		ctk.CTkButton(self, text="Apply Text Overlay", command=self.apply_overlay, 
					height=40, font=ctk.CTkFont(size=14, weight="bold")).grid(row=4, column=0, padx=20, pady=20, sticky="ew")
	...
	def select_coordinates(self):
		...
		self.status_label.configure(text="Click on image to select position, press 'q' to confirm", text_color="blue")
		import cv2
		...
		if selected[0]:
			...
			self.status_label.configure(text=f"Position selected: {selected[0]}", text_color="green")
	...
	def apply_overlay(self):
		...
		try:
			...
			# Show the output image in preview
			basename = os.path.basename(self.image_path)
			name, ext = os.path.splitext(basename)
			output_path = f"./outputs/{name}_edited{ext}"
			if os.path.exists(output_path):
				self.preview_widget.load_image(output_path)
				self.status_label.configure(text="✓ Image saved to outputs/", text_color="green")
				messagebox.showinfo("Success", f"Image saved to {os.path.basename(output_path)}")
			else:
				self.status_label.configure(text="✓ Image saved to outputs/", text_color="green")
				messagebox.showinfo("Success", "Image saved to outputs/")
...
class CreateTemplateTab(ctk.CTkFrame):
	...
	def __init__(self, master, preview_widget, **kwargs):
		...
		ctk.CTkLabel(info_frame, text="Create a reusable template by marking positions on an image", 
		...
		ctk.CTkLabel(name_frame, text="Template Name:").pack(side="left", padx=5)
		...
		# Create button
		ctk.CTkButton(self, text="Create Template (Click on Image)", command=self.create_template,
					height=40, font=ctk.CTkFont(size=14, weight="bold")).grid(row=3, column=0, padx=20, pady=20, sticky="ew")
		
		# Info text
		info = ctk.CTkTextbox(self, height=100, fg_color="gray20")
	...
	def create_template(self):
		...
		try:
			self.status_label.configure(text="Click on image to mark positions...", text_color="blue")
			fn.make_coordinates_template(self.image_path, template_name, max_width=1280, max_height=720)
			self.status_label.configure(text=f"✓ Template '{template_name}' created!", text_color="green")
			...
			self.status_label.configure(text="Error creating template", text_color="red")
...
class ApplyTemplateTab(ctk.CTkFrame):
	...
	def __init__(self, master, preview_widget, **kwargs):
		...
		ctk.CTkButton(template_frame, text="Refresh", command=self.refresh_templates, width=100).pack(side="right", padx=5)
		...
		# Apply button
		ctk.CTkButton(buttons_frame, text="Apply Template to Image", command=self.apply_template,
					height=40, font=ctk.CTkFont(size=14, weight="bold")).pack(side="left", fill="x", expand=True, padx=(0, 5))
		...
		self.update_button = ctk.CTkButton(buttons_frame, text="Update Template", command=self.update_template,
					height=40, font=ctk.CTkFont(size=14, weight="bold"), fg_color="#2196F3", state="disabled")
	...
	def refresh_templates(self):
		...
		if templates:
			...
			self.status_label.configure(text="No templates found", text_color="orange")
	...
	def on_template_selected(self, template_name):
		...
		try:
			...
			for point_name, point_data in self.template_data.items():
				frame = ctk.CTkFrame(self.text_inputs_frame, fg_color="gray25", border_width=1)
				...
				ctk.CTkLabel(main_frame, text=label_text, font=ctk.CTkFont(weight="bold"), width=150, anchor="w").pack(side="left", padx=(0, 10))
				
				# Text input
				ctk.CTkLabel(main_frame, text="Text:", width=40).pack(side="left", padx=(0, 5))
				...
				# Font size
				ctk.CTkLabel(main_frame, text="Size:").pack(side="left", padx=(0, 5))
				...
				# Font style
				ctk.CTkLabel(main_frame, text="Style:").pack(side="left", padx=(0, 5))
				font_style_entry = ctk.CTkEntry(main_frame, width=60)
				font_style_val = point_data.get("font_style", "normal")
				font_style_entry.insert(0, font_style_val)
				font_style_entry.pack(side="left")
				self.font_style_entries[point_name] = font_style_entry
				...
				font_style_entry.bind("<KeyRelease>", lambda e: self.check_for_changes())
			
			self.status_label.configure(text=f"Template '{template_name}' loaded with {len(self.template_data)} points", text_color="green")
	...
	def check_for_changes(self):
		...
		for point_name in self.original_font_data.keys():
			if point_name in self.font_size_entries:
				...
				current_style = self.font_style_entries[point_name].get().strip()
	...
	def apply_template(self):
		...
		try:
			...
			fn.apply_template_to_image(
				...
				text_color="black",  # Will use per-point colors from template
				font_size=20  # Will use per-point sizes from template
			)
			...
			if os.path.exists(output_path):
				self.preview_widget.load_image(output_path)
				self.status_label.configure(text="✓ Template applied! Image saved to outputs/", text_color="green")
				messagebox.showinfo("Success", f"Image saved to {os.path.basename(output_path)}")
			else:
				self.status_label.configure(text="✓ Template applied! Image saved to outputs/", text_color="green")
				messagebox.showinfo("Success", "Image saved to outputs/")
	...
	def update_template(self):
		...
		try:
			...
			for point_name in self.template_data.keys():
				...
				font_style = self.font_style_entries[point_name].get().strip()
			...
			self.status_label.configure(text=f"✓ Template '{template_name}' updated!", text_color="green")
...
class PixelTyperApp(ctk.CTk):
	...
	def __init__(self):
		...
		# Header
		header_frame = ctk.CTkFrame(left_frame, height=50, fg_color="transparent")
		header_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=10)
		
		ctk.CTkLabel(header_frame, text="PixelTyper", 
		...
		ctk.CTkLabel(right_frame, text="Image Preview", 
					font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0, padx=10, pady=10)
		
		self.preview_widget = ImagePreviewWidget(right_frame)
		self.preview_widget.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
		...
		self.simple_overlay_tab = SimpleOverlayTab(self.tab_simple, self.preview_widget)
		self.simple_overlay_tab.pack(fill="both", expand=True)
		
		self.create_template_tab = CreateTemplateTab(self.tab_create, self.preview_widget)
		self.create_template_tab.pack(fill="both", expand=True)
		
		self.apply_template_tab = ApplyTemplateTab(self.tab_apply, self.preview_widget)
